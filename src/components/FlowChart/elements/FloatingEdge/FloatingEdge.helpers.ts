...
export const getNodeIntersection = (intersectionNode: Node, targetNode: Node): XYPosition => {
  ...
  const x1 = (targetPosition as XYPosition).x + w;
  const y1 = (targetPosition as XYPosition).y + h;
...
export const getEdgePosition = (node: Node, intersectionPoint: XYPosition): Position => {
  const n = {
    ...node.positionAbsolute as XYPosition,
    ...node
  };
